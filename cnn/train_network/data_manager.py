...
class DataManager:
    ...
    def next_batch(self):
        num_proteins = (
            self.batch_size
            if self.batch_size <= self.number_proteins - self.active_protein
            else self.number_proteins - self.active_protein
        )
        protein_maps = np.ndarray(
            (num_proteins, self.boxSize, self.boxSize, self.boxSize)
        )
        ss_labels = np.ndarray((num_proteins, self.boxSize, self.boxSize, self.boxSize))
        backbone_labels = np.ndarray(
            (num_proteins, self.boxSize, self.boxSize, self.boxSize)
        )
