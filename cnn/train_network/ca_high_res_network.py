...
def ca_network_definition(protein_maps, sheet, helix, backbone_predictions):
    protein_maps = tf.reshape(
        protein_maps, shape=[-1, params.box_size, params.box_size, params.box_size, 1]
    )
    sheet = tf.reshape(
        sheet, shape=[-1, params.box_size, params.box_size, params.box_size, 1]
    )
    helix = tf.reshape(
        helix, shape=[-1, params.box_size, params.box_size, params.box_size, 1]
    )
    backbone_predictions = tf.reshape(
        backbone_predictions,
        shape=[-1, params.box_size, params.box_size, params.box_size, 1],
    )
    x = tf.concat(
        [
            protein_maps,
            tf.cast(sheet, dtype=tf.float32),
            tf.cast(helix, dtype=tf.float32),
            tf.cast(backbone_predictions, dtype=tf.float32),
        ],
        4,
    )
